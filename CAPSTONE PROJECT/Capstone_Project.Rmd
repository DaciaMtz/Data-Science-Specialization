...
sample_selection <- rbinom(nrow(all_data), size = 1, prob = 0.7)  # 70% of the data
...
unigram_probs <- as.data.table(unigram_probs) # Convert data frames to data.tables
saveRDS(unigram_probs, "unigram_probs.rds") # Save the probabilities
...
bigram_probs <- as.data.table(bigram_probs) # Convert data frames to data.tables
saveRDS(bigram_probs, "bigram_probs.rds") # Save the probabilities

# Trigrams  -------------------------------------
set.seed(123)  # For reproducibility
sample_selection <- rbinom(nrow(all_data), size = 1, prob = 0.5)  # 50% of the data
subsample_data <- all_data[sample_selection == 1, ]
...
trigram_probs <- as.data.table(trigram_probs) # Convert data frames to data.tables
saveRDS(trigram_probs, "trigram_probs.rds") # Save the probabilities
...
fourgram_probs <- as.data.table(fourgram_probs) # Convert data frames to data.tables
saveRDS(fourgram_probs, "fourgram_probs.rds") # Save the probabilities
